...
# 1. Prepariamo i dati escludendo i duplicati esatti per evitare VIF = Inf
# (Rimuoviamo le originali WE_ che sono identiche a Temp, WindSpeed, ecc.)
df_vif_full <- df %>% 
...
# 2. HEATMAP CORE
# Selezioniamo il target e i 4 core
df_corr_final <- df %>% 
...
```{r model_performance}
# Estraiamo l'R-quadrato
r_sq <- summary(model_meteo)$r.squared
adj_r_sq <- summary(model_meteo)$adj.r.squared
```

**Model Performance:**
The meteorological model explains **`r round(r_sq * 100, 1)`%** of the total variance in PM10 levels (Multiple $R^2$ = `r round(r_sq, 3)`). This indicates a strong fit, considering the high daily variability of atmospheric data.
...
```{r model_gamma}
# 1. Fit del modello Gamma (usiamo Y originale, non log)
# Aggiungiamo 0.001 per sicurezza statistica (Y deve essere > 0)
model_gamma <- glm((Y + 0.001) ~ Temp + WindSpeed + Precipitation + Humidity, 
                   data = df, 
                   family = Gamma(link = "log"))

# 2. Tabella di confronto dei coefficienti Beta
...
Finally, we evaluate the overall explanatory power of our meteorological baseline. The Multiple $R^2$ for the core model is **0.399**.
