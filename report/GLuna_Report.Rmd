...
**Variable Selection (WE Variables):**

From the various meteorological (WE) variables available in the Agrimonia dataset, we selected four primary drivers (**Temperature, Wind Speed, Precipitation, and Humidity**) based on established atmospheric physics. These variables represent the fundamental mechanisms of washout (precipitation) and ventilation (wind speed and temperature-driven boundary layer dynamics). At this stage, we exclude seasonality and human factors (Covid) to focus exclusively on these natural dispersion mechanisms.
